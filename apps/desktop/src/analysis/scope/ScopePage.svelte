<script lang="ts">
  ...
  export let onConnection: (connection: ConnectionInfo | undefined) => void = () => undefined;
  ...
  $: if (connection && selectedIds.size === 0) selectedIds = new Set(connection.channels.filter((c) => c.supportsFast).slice(0, 2).map((c) => c.id));
  ...
  function resetScope() { scopeConfig = undefined; snapshot = undefined; selectedIds = new Set<number>(); rebuildPlot([]); }
  function connected(next: ConnectionInfo) { resetScope(); onConnection(next); }
  function disconnected() { resetScope(); onConnection(undefined); }

  function toggleChannel(id: number) {
    if (!connection) return;
    const next = new Set(selectedIds);
    if (next.has(id)) next.delete(id); else if (next.size < connection.fastMaxChannels) next.add(id);
    selectedIds = next; scopeConfig = undefined; snapshot = undefined;
  }

  async function ensureConfigured(): Promise<boolean> {
    if (scopeConfig) return true;
    if (selectedIds.size === 0) { onError("Select at least one FAST channel."); return false; }
    try { scopeConfig = await configureScope(Array.from(selectedIds)); rebuildPlot(scopeConfig.channels); return true; }
  ...
  function rebuildPlot(channels: ScopeConfig["channels"]) {
    if (!plotHost) return;
    const rect = plotHost.getBoundingClientRect(); plot?.destroy();
    const series: uPlot.Series[] = [{}, ...channels.map((c, i) => ({ label: c.symbol, stroke: traceColors[i % traceColors.length], width: 1.25 }))];
    plot = new uPlot({ width: Math.max(420, Math.floor(rect.width)), height: Math.max(260, Math.floor(rect.height)), legend: { show: false }, cursor: { drag: { x: true, y: false } }, scales: { x: { time: false } }, axes: [
      { stroke: "#8c8c8c", grid: { stroke: "#2a2d2e", width: 1 }, ticks: { stroke: "#3a3d41" } },
      { stroke: "#8c8c8c", grid: { stroke: "#2a2d2e", width: 1 }, ticks: { stroke: "#3a3d41" } },
    ], series }, [[], ...channels.map(() => [])] as uPlot.AlignedData, plotHost);
  }

  function resizePlot() { if (!plot || !plotHost) return; const rect = plotHost.getBoundingClientRect(); plot.setSize({ width: Math.max(420, Math.floor(rect.width)), height: Math.max(260, Math.floor(rect.height)) }); }
  function latestValue(index: number): string {
    if (!snapshot || !scopeConfig || snapshot.series[index]?.length === 0) return "—";
    const values = snapshot.series[index]; const value = values[values.length - 1]; const unit = scopeConfig.channels[index]?.unit ?? "";
  ...
  onMount(() => {
    ...
    rebuildPlot([]); resizeObserver = new ResizeObserver(resizePlot); resizeObserver.observe(plotHost); refreshTimer = setInterval(refreshSnapshot, 50);
    return () => { if (refreshTimer) clearInterval(refreshTimer); resizeObserver?.disconnect(); plot?.destroy(); split?.destroy(); };
  });
</script>

<section class="page-toolbar"><div class="page-title">SCOPE</div><div class="toolbar-actions">
...
<div class="scope-shell">
  <aside id="scope-sidebar" class="scope-sidebar">
    <ConnectionPanel {connection} onConnected={connected} onDisconnected={disconnected} {onError} />
    <section class="side-section"><div class="section-heading">CHANNELS</div><div class="channel-list">
      ...
      {/each}{:else}<div class="empty-hint">Connect a device to discover Plot channels.</div>{/if}
    </div></section>
  ...
  <section id="scope-workspace" class="scope-workspace">
    ...
    <div class="plot-header">{#if scopeConfig}{#each scopeConfig.channels as channel, index}<div class="trace-key"><span class="trace-mark" style={`background:${traceColors[index % traceColors.length]}`}></span>{channel.symbol}<span class="value">{latestValue(index)}</span></div>{/each}{:else}<div class="plot-placeholder">Select FAST channels and press Run.</div>{/if}<div class="plot-meta">{snapshot?.sampleCount ?? 0} samples · loss {snapshot?.lostFrames ?? 0}</div></div>
